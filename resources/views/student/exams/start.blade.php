    ...
    <div class="exam-shell">
        ...
        <form id="examForm" method="POST" action="{{ route('student.exams.submit', $exam->id) }}" class="mt-5">
            ...
            <input type="hidden" name="attempt_id" id="attempt_id"
                   value="{{ $attemptId ?? session('exam_attempt_'.$exam->id.'.attempt_id') }}">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
                ...
                <aside class="xl:col-span-4 side-glass">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="submit" id="submitBtn" class="btn btn-success text-sm px-3 py-2">Submit Exam</button>
                    </div>
...
document.getElementById('question_order').value = JSON.stringify(questions.map(q => q.id));
document.getElementById('option_order').value = JSON.stringify(
    Object.fromEntries(questions.map(q => [q.id, q.options.map(o => o.id)]))
);
    ...
    try {
        await fetch("{{ route('student.exams.autosave', $exam->id) }}", {
            ...
            body: JSON.stringify({
                ...
                answers,
                flagged: Object.keys(flagged).filter(qid => flagged[qid])
            })
        });
    } catch {}
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        appAlert(`Tab switch detected. Warning ${warnings}/4`, {
            title: 'Exam Warning',
            eyebrow: 'Attention'
        });
...
fetch("{{ route('student.exams.load_saved', $exam->id) }}")
    ...
    .then(data => {
        ...
        Object.entries(answers).forEach(([q, o]) => syncHiddenAnswer(q, o));
        flagged = {};
        (data.flagged || []).forEach(q => flagged[q] = true);
        renderQuestion();
    })
